...
The current analysis includes data on `r nrow(df)` lymphatic structures from
`r n_distinct(df$TissueID)` mice, belonging to `r n_distinct(df$Strain)` strains,
`r n_distinct(df$Age)` age categories and `r n_distinct(df$Sex)` genders, for a
total of `r n_distinct(interaction(df$Strain, df$Age, df$Sex))` groups (@tbl-stats).
