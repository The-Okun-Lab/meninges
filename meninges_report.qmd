...
-   Ask Amit's opinion about current results and discuss next steps
-   Ask Amit how angle is calculated and implement angle computation in this script, so that new images can be easily integrated in this analysis

```{r}
#| label: data
figure <- readPNG("data/opac_meninges.png")
df <- read.csv2("data/processed_data.csv")
...
ggplot(df, aes(x = X, y = Y)) +
  ...
  #  geom_text(data = subset(df, Size > my_label_threshold), aes(label = Label),
  #            size = my_label_size, colour = my_label_colour) +
