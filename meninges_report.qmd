...
df <- df %>%
  ...
  mutate(Phenotype = as.factor(Phenotype),
         Sex = factor(Sex, c("Males", "Females")))

if (params$str == "Dp1tyb") {
  
  df$Age[df$Age == "young"] <- "2 months"
  df$Age[df$Age == "middle"] <- "10-15 months"
  
  levs <- c("Males.2 months", "Males.10-15 months",
          "Females.2 months", "Females.10-15 months")
  
  df <- df %>%
    mutate(Age = factor(Age, c("2 months", "10-15 months")),
           Sex.Age = factor(interaction(Sex, Age, sep = "."), levs))
  
} else {
  
  df$Age[df$Age == "young"] <- "1 month"
  df$Age[df$Age == "middle"] <- "12 months"
  df$Age[df$Age == "old"] <- "18+ months"
  
  levs <- c("Males.1 month", "Males.12 months", "Males.18+ months",
          "Females.1 month", "Females.12 months", "Females.18+ months")
  
  df <- df %>%
    mutate(Age = factor(Age, c("1 month", "12 months", "18+ months")),
           Sex.Age = factor(interaction(Sex, Age, sep = "."), levs))
...
ggplot(df, aes(x = X, y = Y)) +
  ...
  geom_text(data = text_df, aes(label = Label),
            x = -side_lim + 2.4, y = upper_lim + 0.3, size = 3) +
  scale_fill_gradient2(low = my_low, high = my_high, limits=c(0, 40)) +
...
ggplot(df, aes(x = X, y = Y)) +
  ...
  annotation_custom(rasterGrob(figure, 
                    ...
                    -meninges_bottom, meninges_height) +
  geom_text(data = text_df, aes(label = Label),
            x = -side_lim + 1.9, y = upper_lim + 0.3, size = 3) +
...
```{r}
#| label: fig-ndens
#| fig-cap: Density plot of TLSs around meninges. Colour indicates the fraction of structures found in each pixel normalised by group maximum. Each panel corresponds to a mice group based on sex and age category.
#| fig-width: !expr 5 + 2.75 * (n_distinct(df$Age) - 1)
#| fig-height: !expr 3.5 * n_distinct(df$Sex)
ggplot(df, aes(x = X, y = Y)) +
  lims(x = c(-side_lim, side_lim), y = c(lower_lim, upper_lim)) +
  ...
  geom_text(data = text_df, aes(label = Label),
            x = -side_lim + 1.9, y = upper_lim + 0.3, size = 3) +
  scale_fill_distiller(palette = my_palette, direction = my_dir) +
  labs(x = "X in mm", y = "Y in mm", fill = "NDensity") +
  facet_grid(Sex ~ Age) +
  theme_bw()
```
...
```{r}
#| label: fig-ndens-pheno
#| fig-cap: Density plot of tertiary lymphatic structures around meninges by phenotype (1, 2 or 3). Colour indicates the fraction of structures found in each pixel normalised by group maximum. Rows correspond to a mice group based on sex and age category, whereas columns represent phenotypes.
#| fig-width: 10
#| fig-height: !expr 3 * n_distinct(df$Sex.Age)
ggplot(df, aes(x = X, y = Y)) +
  lims(x = c(-side_lim, side_lim), y = c(lower_lim, upper_lim)) +
